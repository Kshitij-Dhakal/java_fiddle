...
public class GroupRoutes {
  ...
  private static List<RouteDetail> groupRoutes(List<RouteDetail> routes) {
    ...
    routes.stream()
        ...
        .forEach(
            (path, list) -> {
              var handlers = list.stream().map(RouteDetail::handler).collect(Collectors.toSet());
              if (handlers.size() == 1) {
    ...
    // some routes like /conversation/:id0 isn't added in first2Part because similar routes point to
    // multiple APIs
    // but is later added in first3Map or remaining
    var filteredRoutes = new HashMap<String, RouteDetail>();
    convertedRoutes.values().stream()
        .sorted(Comparator.comparingInt(o -> o.route().split("/").length))
        .forEach(
            it -> {
              var matchingFirstPartExists =
                  filteredRoutes.containsKey(firstPart(it))
                      && filteredRoutes.get(firstPart(it)).handler().equals(it.handler());
              var matchingFirst2PartExists =
                  filteredRoutes.containsKey(first2Part(it))
                      && filteredRoutes.get(first2Part(it)).handler().equals(it.handler());
              var matchingFirst3PartExists =
                  filteredRoutes.containsKey(first3Part(it))
                      && filteredRoutes.get(first3Part(it)).handler().equals(it.handler());
              if (!matchingFirstPartExists
                  && !matchingFirst2PartExists
                  && !matchingFirst3PartExists) {
    ...
    return new ArrayList<>(filteredRoutes.values())
        .stream()
            .sorted(
                (o1, o2) ->
                    Integer.compare(o2.route().split("/").length, o1.route().split("/").length))
            .toList();
  }
  ...
  private static String firstNPart(RouteDetail it, int maxSize) {
    return Arrays.stream(it.route().split("/")).limit(maxSize).collect(Collectors.joining("/"));
  }
